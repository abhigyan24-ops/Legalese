/**
 * groqAI.js — Groq API Client for Rights Quest & Advocate Portal
 *
 * Powers:
 * 1. AI Legal Answer Bot — generates child-safe legal answers for Q&A
 * 2. AI Content Moderation — screens posts before publishing
 * 3. Advocate Legal Answer Drafter — assists lawyers with statutory legal citations
 *
...
export async function moderateContent(text) {
  ...
  try {
    ...
    try {
      ...
      const isUnsafe = /unsafe|false/i.test(content);
      return { safe: !isUnsafe };
    }
  } catch (err) {
    console.warn('Groq moderation request failed:', err.message);
    return { safe: true };
  }
}

/**
 * AI Drafting Assistant for Advocate Dashboard
 * Drafts an authoritative, statutory legal response based on Indian constitutional & child rights laws
 * @param {string} question - The student's question
 * @param {string} storyTopic - The topic/chapter (e.g. "Right to Education", "POCSO Act")
 * @returns {Promise<string>} - Draft legal answer for the advocate to edit and approve
 */
